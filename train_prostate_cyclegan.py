"""Train CycleGAN for bidirectional T2 ↔ ADC translation on prostate158.

Mirrors train_cv_cyclegan.py. Same caveat: data is paired same-grid, but we
sample each modality independently (CycleGAN's "unpaired" regime). This tests
whether cycle-consistency learning matches or improves on the paired pix2pix.

A = T2 (input modality), B = ADC (target modality).

Usage:
    python train_prostate_cyclegan.py --epochs 100 --batch 4 \
        --out prostate_models/cyclegan_t2_adc
"""
...
class _SingleModality2D(Dataset):
    """Yields one modality of slices from prostate_data for unpaired sampling.

    We trick prostate_data.get_paired_2d_dataset into returning the wanted
    modality on BOTH the "input" and "target" fields by setting both
    input_modality and target_modality to the same value.
    """
    ...
    def __getitem__(self, i):
        item = self.base[i]
        return {"image": item["input"]}     # (1, H, W) in [-1, 1]
...
def save_translation_grid(real_A, fake_B, rec_A, real_B, fake_A, rec_B,
                          path: Path):
    """6-col grid (real_A | fake_B | rec_A | real_B | fake_A | rec_B). A=T2, B=ADC."""
...
def train(args):
    ...
    # A = T2, B = ADC  (unpaired sampling even though source is paired)
    A_train = _SingleModality2D("train", "t2",  args.image_size, augment=True)
    B_train = _SingleModality2D("train", "adc", args.image_size, augment=True)
    A_val   = _SingleModality2D("val",   "t2",  args.image_size, augment=False)
    B_val   = _SingleModality2D("val",   "adc", args.image_size, augment=False)
    print(f"[data] T2:  train={len(A_train)} val={len(A_val)}")
    print(f"[data] ADC: train={len(B_train)} val={len(B_val)}")
...
def parse_args():
    ...
    p.add_argument("--out", default="prostate_models/cyclegan_t2_adc")
