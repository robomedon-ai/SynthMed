"""Train SPADE-multi-class for vessel-mask → CTA on the TopBrain dataset.

L1 = 0 by default (lesson from PASD SPADE: L1 hurts mask faithfulness).
TTUR (G=1e-4, D=4e-4), hinge GAN + feature-matching + VGG perceptual, EMA.

Usage:
    python train_cv_spade.py --epochs 100 --batch 4 \
        --out cv_models/spade_mask2ct
"""
...
def _mask_vis(mask_long: torch.Tensor) -> np.ndarray:
    """(B, H, W) long → uint8 visualization (raw class IDs scaled to 0-255)."""
    m = mask_long.clamp(0, sp.CV_LABEL_NC - 1).float()
    m = (m / max(sp.CV_LABEL_NC - 1, 1)) * 255
    return m.byte().cpu().numpy()


def save_sample_grid(masks, reals, fakes, path: Path):
    """3-col grid (mask | real CT | fake CT)."""
    msk = _mask_vis(masks)
...
class EMAWrap:
    """Tiny EMA helper, mirrors train_pasd_spade.EMAWrap."""
...
def train(args):
    ...
    val_batch = next(iter(DataLoader(val_ds, batch_size=min(8, len(val_ds)),
                                     shuffle=False)))
    val_masks_long = val_batch["mask_ct"].to(device)
    val_masks_oh   = sp.one_hot_mask(val_masks_long)
    val_ct         = val_batch["ct"].to(device)
    z_fixed = torch.randn(val_ct.size(0), 256, device=device)

    # Models — input: 41-ch one-hot mask, output: 1-ch grayscale
    G = sp.CvSPADEGenerator(mask_nc=sp.CV_LABEL_NC, image_ch=1,
                            ngf=args.ngf, z_dim=256).to(device)
    D = sp.FeaturePatchDiscriminator(in_ch=sp.CV_LABEL_NC + 1,
                                     ndf=64).to(device)
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for batch in train_loader:
            real = batch["ct"].to(device, non_blocking=True)         # (B,1,H,W)
            mask_long = batch["mask_ct"].to(device, non_blocking=True)
            mask_oh = sp.one_hot_mask(mask_long)                     # (B,41,H,W)
        ...
        if epoch % args.sample_every == 0 or epoch == args.epochs:
            ...
            save_sample_grid(val_masks_long, val_ct, fake_val,
...
def parse_args():
    ...
    p.add_argument("--out", default="cv_models/spade_mask2ct")
    ...
    p.add_argument("--l1_weight", type=float, default=0.0,
                   help="L1 weight (default 0 — lesson from PASD SPADE)")
