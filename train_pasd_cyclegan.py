"""Train CycleGAN for BTFE ↔ TSE cross-modality translation.

Usage:
    python train_pasd_cyclegan.py --epochs 100 --batch 4 \
        --out pasd_models/cyclegan_btfe_tse

Unpaired training: each iteration pulls a batch of real BTFE and a separate
batch of real TSE (independently sampled). The model never sees paired
slices, mirroring the standard CycleGAN setup.
"""
...
def save_translation_grid(real_A, fake_B, recon_A, real_B, fake_A, recon_B,
                          path: Path):
    """6-column grid: (real_A | fake_B | rec_A | real_B | fake_A | rec_B)."""
    cols = [real_A, fake_B, recon_A, real_B, fake_A, recon_B]
    cols = [_denorm(c) for c in cols]
    n, H, W, _ = cols[0].shape
    grid = np.zeros((n * H, 6 * W, 3), dtype=np.uint8)
    for i in range(n):
...
def train(args):
    ...
    # ── Unpaired data: two independent loaders, one per modality ──
    A_train = pd.get_pasd_dataset("BTFE", "train", image_size=args.image_size, augment=True)
    B_train = pd.get_pasd_dataset("TSE",  "train", image_size=args.image_size, augment=True)
    A_val   = pd.get_pasd_dataset("BTFE", "val",   image_size=args.image_size, augment=False)
    B_val   = pd.get_pasd_dataset("TSE",  "val",   image_size=args.image_size, augment=False)
    print(f"[data] BTFE: train={len(A_train)} val={len(A_val)}")
    print(f"[data] TSE : train={len(B_train)} val={len(B_val)}")
    ...
    # Fixed val pair for sample grids
    A_val_batch = next(iter(DataLoader(A_val, batch_size=min(4, len(A_val)), shuffle=False)))
    B_val_batch = next(iter(DataLoader(B_val, batch_size=min(4, len(B_val)), shuffle=False)))
    fixed_real_A = A_val_batch["image"].to(device)
    fixed_real_B = B_val_batch["image"].to(device)

    # ── Models ──
    G_AB = cg.ResnetGenerator(3, 3, ngf=args.ngf, n_blocks=9).to(device)
    G_BA = cg.ResnetGenerator(3, 3, ngf=args.ngf, n_blocks=9).to(device)
    D_A  = cg.PatchDiscriminator(in_ch=3, ndf=64).to(device)
    D_B  = cg.PatchDiscriminator(in_ch=3, ndf=64).to(device)
    for m in (G_AB, G_BA, D_A, D_B):
        cg.init_weights(m)
    print(f"[arch] G_AB=G_BA: {sum(p.numel() for p in G_AB.parameters())/1e6:.1f}M each")
    ...
    opt_D = torch.optim.Adam(
        ...
        lr=args.lr, betas=(0.5, 0.999))
    # Linear LR decay in second half (standard CycleGAN recipe)
    def lr_lambda(epoch):
    ...
    for epoch in range(start_ep, args.epochs + 1):
        ...
        # Pair the two unpaired streams via zip — drops the shorter when exhausted
        ...
        for A_batch in A_loader:
            ...
            # ── G step: cycle + identity + adversarial ──
            ...
            with torch.amp.autocast("cuda", enabled=args.amp):
                ...
                fake_A = G_BA(real_B)
                rec_B  = G_AB(fake_A)
                idt_A  = G_BA(real_A)   # G_BA should preserve A
                idt_B  = G_AB(real_B)

                loss_g_gan = (lsgan(D_B(fake_B), True) +
                              lsgan(D_A(fake_A), True))
                loss_g_cyc = (l1(rec_A, real_A) +
                              l1(rec_B, real_B)) * args.lambda_cycle
                loss_g_id  = (l1(idt_A, real_A) +
                              l1(idt_B, real_B)) * args.lambda_identity
                loss_G = loss_g_gan + loss_g_cyc + loss_g_id
            scaler_G.scale(loss_G).backward()
            scaler_G.step(opt_G); scaler_G.update()

            # ── D step: real vs (buffered) fake ──
        ...
        print(f"[ep {epoch:3d}/{args.epochs}] "
              ...
              f"lr={stats['lr']:.1e}  t={stats['time_s']:.1f}s", flush=True)

        if epoch % args.sample_every == 0 or epoch == args.epochs:
            G_AB.eval(); G_BA.eval()
            with torch.no_grad():
                fB = G_AB(fixed_real_A)
                rA = G_BA(fB)
                fA = G_BA(fixed_real_B)
                rB = G_AB(fA)
            save_translation_grid(fixed_real_A, fB, rA,
                                  fixed_real_B, fA, rB,
...
def parse_args():
    ...
    p.add_argument("--out", default="pasd_models/cyclegan_btfe_tse")
