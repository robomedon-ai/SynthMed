"""Train the prostate LDM (T2 → ADC via latent diffusion) on prostate158.

Mirrors train_cv_ldm.py — v-prediction, AMP, EMA, sample grids, --resume.

Usage (after train_prostate_vae.py finishes):
    python train_prostate_ldm.py \
        --vae_ckpt prostate_models/vae/latest.pt \
        --steps 30000 --batch 16 \
        --out prostate_models/ldm_t2_adc
"""
...
def save_grid(t2s, adcs, fakes, path: Path):
    """3-col grid: (real T2 | real ADC | fake ADC). Inputs are 3-channel."""
    t2_np  = _denorm_uint8(t2s)
    adc_np = _denorm_uint8(adcs)
...
def load_vae_with_finetuned_decoder(vae_ckpt: str, device: str):
    """Load SD-VAE then overlay our fine-tuned prostate decoder weights."""
...
def get_paired_3ch_loader(split: str, image_size: int, batch: int, workers: int):
    """Wrap prostate_data with grayscale→RGB replication for SD-VAE."""
    ...
    class _Paired3ch(torch.utils.data.Dataset):
        ...
        def __getitem__(self, i):
            item = base[i]
            return {"t2":  item["input"].repeat(3, 1, 1),
...
def train(args):
    ...
    fixed_t2  = fixed_val["t2"].to(device)
    fixed_adc = fixed_val["adc"].to(device)
    ...
    while step < args.steps:
        ...
        with torch.amp.autocast("cuda", enabled=args.amp):
            loss, parts = ldm.training_step(unet, vae, t2, adc, train_sched,
        ...
        ema.update(unet)
        log_buf["mse"] += parts["mse"] * t2.size(0); log_buf["n"] += t2.size(0)
        step += 1
        ...
        if step % args.sample_every == 0 or step == args.steps:
            ...
            with torch.no_grad():
                fake = ldm.sample(unet, vae, fixed_t2,
...
def parse_args():
    ...
    p.add_argument("--vae_ckpt", required=True,
                   help="Path to fine-tuned prostate VAE (or '' to use pretrained SD-VAE)")
    p.add_argument("--out", default="prostate_models/ldm_t2_adc")
